frappe.pages['supplier-collection-form'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __('Supplier Collection Form'),
		single_column: true
	});
    ...
    function get_data(filters) {
        ...
        frappe.call({
            method: 'agricultural_marketing.agricultural_marketing.page.supplier_collection_form.supplier_collection_form.execute',
            args : {
    ...
    let $btn = page.set_primary_action( __('Generate Supplier Collection Form'), () => { get_data(page.fields_dict) });
    let $btnPDF = page.set_secondary_action( __('Open PDF'), () => { open_pdf(page.fields_dict) });

}

frappe.pages['supplier-collection-form'].on_page_show = function(wrapper) {
